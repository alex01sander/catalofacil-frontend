  ...
  return <Sheet>
      <SheetTrigger asChild>
        <Button variant="ghost" size="sm" className="relative">
          ...
          {totalItems > 0 && <Badge className="absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs bg-violet-950">
              {totalItems}
            </Badge>}
        </Button>
      </SheetTrigger>
        ...
        <div className="flex flex-col h-full">
          {items.length === 0 ? <div className="flex-1 flex items-center justify-center">
              ...
              <div className="flex-1 overflow-y-auto py-4">
                {items.map(item => <div key={item.id} className="flex items-center gap-4 py-4 border-b">
